document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchSatellitePosition(satId) {
    ...
    const observerAlt = 0;
    const url = `/isro/api/satellite-position/?satid=${satId}&lat=${observerLat}&lon=${observerLon}&alt=${observerAlt}`;
  ...
  async function drawOrbitPath(satId) {
    ...
    const url = `/isro/api/satellite-path/?satid=${satId}&lat=${observerLat}&lon=${observerLon}&alt=${observerAlt}&secs=${totalSeconds}&start=${startTime}`;
  ...
  passForm?.addEventListener("submit", async function (e) {
    ...
    try {
      const res = await fetch(`/isro/api/satellite-passes/?lat=${latInput}&lon=${lonInput}&satid=${satId}`);
